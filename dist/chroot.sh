...
export "LD_PRELOAD=`pwd`/libfakechroot.so `pwd`/libfakedns.so `pwd`/libdisableselinux.so `pwd`/libandroid-shmem.so"
export FAKECHROOT_EXCLUDE_PATH=/dev:/proc:/sys:/system:$SDCARD
